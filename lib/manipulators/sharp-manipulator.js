...
(function() {
  var SharpManipulator, enrich_params, sharp,
  ...
  SharpManipulator = (function() {
    ...
    SharpManipulator.prototype.manipulate = function(callback) {
      var calculated_width, degrees, height, height_ratio, image, percent_factor, region_width, width, width_ratio;
      this.params = enrich_params(this.params, this.info);
      image = sharp(this.temp_bmp);
      if (this.params.size !== 'full') {
        if ((this.params.size.w != null) && (this.params.size.h != null)) {
          if (this.params.size.type === 'sizeByConfinedWh') {
            width = this.params.region === 'full' ? this.info.width : this.params.region.w;
            height = this.params.region === 'full' ? this.info.height : this.params.region.h;
            width_ratio = width / this.params.size.w;
            height_ratio = height / this.params.size.h;
            console.log(this.params);
            console.log([width_ratio, height_ratio]);
            if (width_ratio > height_ratio) {
              image.resize(this.params.size.w);
            } else {
              image.resize(void 0, this.params.size.h);
            }
          } else {
            image.resize(this.params.size.w, this.params.size.h);
          }
        } else if (this.params.size.w != null) {
          image.resize(this.params.size.w);
        } else if (this.params.size.h != null) {
